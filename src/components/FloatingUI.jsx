...
export default function FloatingUI() {
  ...
  return (
    <>
      {/* Skroll progress */}
      <div style={{ position: 'fixed', top: 0, left: 0, height: 3, width: `${progress}%`, background: 'linear-gradient(90deg,#a8a8a8,#767676,#2b2b2b)', zIndex: 200, transition: 'width .1s linear' }} />
      ...
      <div style={{ position: 'fixed', right: 18, bottom: 18, zIndex: 190, display: 'flex', flexDirection: 'column', gap: 12, alignItems: 'center' }}>
        <button onClick={toTop} aria-label="Tepaga" title="Tepaga"
          style={{ width: 46, height: 46, borderRadius: '50%', border: 'none', cursor: 'pointer', background: '#fff', color: '#2b2b2b', boxShadow: '0 8px 22px -8px rgba(0,0,0,.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: show ? 1 : 0, transform: show ? 'scale(1)' : 'scale(.7)', pointerEvents: show ? 'auto' : 'none', transition: 'opacity .3s, transform .3s' }}>
        ...
        <a href={`tel:${PHONE}`} aria-label="Qoʻngʻiroq" title="Qoʻngʻiroq"
          style={{ width: 52, height: 52, borderRadius: '50%', background: '#5f5f5f', color: '#fff', boxShadow: '0 10px 26px -8px rgba(43,43,43,.7)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Phone size={22} />
        </a>
